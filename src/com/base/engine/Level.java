...
public class Level
{
	...
	public Level(String levelName, String textureName)
	{
		...
		temp.setTranslation(new Vector3f(7.f, 0.f, 12.f));
		
		//door = new Door(temp, material);
	}
...
//			collisionVector = collisionVector.mul(rectCollide(oldPos2, newPos2, objectSize, doorPos2f, doorSize));
	...
	{
		int texX = value / NUM_TEXTURES; //get green component
		int texY = texX % NUM_TEX_EXP;
		texX /= NUM_TEX_EXP;
		
		float[] result = new float[4];
		
		result[0] = 1.f - (float)texX / (float)NUM_TEX_EXP;
		result[1] = result[0] - 1.f / (float) NUM_TEX_EXP;;
		result[3] = 1.f - (float)texY / (float)NUM_TEX_EXP;
		result[2] = result[3] - 1.f / (float)NUM_TEX_EXP;
	...
	private void addDoor(int x, int y)
	{
		...
		if(!(xDoor ^ yDoor))
		{
			System.err.println("Level generation failed! Door in invalid location at " + x + ", " + y);
			new Exception().printStackTrace();
			System.exit(1);
		}
